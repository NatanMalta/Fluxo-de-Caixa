...
**Extrato da Conta**:
Tela que lista cronologicamente os Lançamentos que afetam uma única Conta — inclui Comum, Ajuste e Transferência, com `sentido` derivado pra Transferência (entrada se `ContaDestinoId == estaConta`, saída se `ContaOrigemId == estaConta`). Tem filtros por intervalo de datas (default "este mês", dia 1 até hoje) e por sentido (Todas / Entradas / Saídas, aplicado no cliente), e ao final mostra os totais de Entradas e Saídas do range filtrado. **Acessada via tap em uma Conta no Dashboard — sub-tela via `Navigator.push`, não 5ª aba no `NavigationBar`** (o bottom nav já tem 4 destinos e 5 ficaria apertado no celular do dono; a 5ª aba também exigiria dropdown de Conta no header, enquanto a sub-tela usa a lista de Contas do Dashboard como índice natural). Toque em um Lançamento abre a edição na aba Lançar via ADR 0004 (mesmo canal do widget "Últimos lançamentos"). O saldo no header é o **atual** da Conta (não o saldo no fim do range filtrado). Reusa `DataInvalidator.lancamentos` e `DataInvalidator.contas` para refresh. Diferente de Balanço (resumo agregado por período) e de Lançar (entrada de dados).
_Avoid_: "transação" (já em avoid do Lançamento), "histórico" (genérico demais, perde a referência à Conta), "extrato bancário" (sugere integração com banco que não temos), "5ª aba" / "aba Extrato" (não é uma aba, é uma sub-tela empilhada).
